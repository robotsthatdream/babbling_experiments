...
class Controller: public Component{
    ...
    public :
        ...
        void client_connect_to_ros(){
                ...
                _pr2_mover.reset(new pr2_mover::PR2_Mover(*cafer_core::ros_nh));
                _move_pr2_arm.reset(new ros::ServiceClient(cafer_core::ros_nh->serviceClient<pr2_mover_utils::move_pr2_arm>("/move_pr2_arm")));
        ...
        void init(){
                ...
                _home_variable_values.insert ( std::pair<std::string, double>("l_shoulder_pan_joint", 1.95));//1.00) );//1.95
                _home_variable_values.insert ( std::pair<std::string, double>("l_shoulder_lift_joint", 0.21));//0.42) );//0.21
                _home_variable_values.insert ( std::pair<std::string, double>("l_upper_arm_roll_joint", 1.08));//0.08) );//1.08
                _home_variable_values.insert ( std::pair<std::string, double>("l_elbow_flex_joint",  -1.8));//-1.44) );//-1.8
                _home_variable_values.insert ( std::pair<std::string, double>("l_forearm_roll_joint", 1.48));//1.44) );//1.48
                _home_variable_values.insert ( std::pair<std::string, double>("l_wrist_flex_joint",  -1.34));//-0.58) );//-1.34
                _home_variable_values.insert ( std::pair<std::string, double>("l_wrist_roll_joint",  -2.87));//-2.30) );//-2.87


                _secondary_home.insert ( std::pair<std::string, double>("r_shoulder_pan_joint", -2.00) );
                _secondary_home.insert ( std::pair<std::string, double>("r_shoulder_lift_joint", 0.36) );
                _secondary_home.insert ( std::pair<std::string, double>("r_upper_arm_roll_joint", -1.44) );
                _secondary_home.insert ( std::pair<std::string, double>("r_elbow_flex_joint",  -2.12) );
                _secondary_home.insert ( std::pair<std::string, double>("r_forearm_roll_joint", -1.90) );
                _secondary_home.insert ( std::pair<std::string, double>("r_wrist_flex_joint",  -1.00) );
                _secondary_home.insert ( std::pair<std::string, double>("r_wrist_roll_joint",  0.32) );

                //        left_home_values_ = {1.00, 0.42, 0.08, -1.44, 1.44, -0.53, -2.30};
                left_home_values_ = {1.95, 0.21, 1.08, -1.8, 1.48, -1.34, -2.87};
                right_home_values_ = {-2.00, 0.36, -1.44, -2.12, -1.90, -1.00, 0.32};
                ...
                _pr2_mover->group->setPlannerId(static_cast<std::string>(_pr2_mover->global_parameters.get_planner_parameters()["planner_id"]));
                _pr2_mover->group->setPlanningTime(std::stod(_pr2_mover->global_parameters.get_planner_parameters()["planning_time"]));

                _pr2_mover->secondary_group->setPlannerId(static_cast<std::string>(_pr2_mover->global_parameters.get_planner_parameters()["planner_id"]));
                _pr2_mover->secondary_group->setPlanningTime(std::stod(_pr2_mover->global_parameters.get_planner_parameters()["planning_time"]));

                ROS_WARN_STREAM("CONTROLLER : The planner ID is : " << static_cast<std::string>(_pr2_mover->global_parameters.get_planner_parameters()["planner_id"]));


                _pr2_mover->global_parameters.set_approach_radius(std::stod(_pr2_mover->global_parameters.get_planner_parameters()["approach_radius"]));
                _approach_radius = _pr2_mover->global_parameters.get_approach_radius();
        ...
        void octomap_manipulation(bool manipulate){
                _pr2_mover->global_parameters.set_adding_octomap_to_acm(manipulate);
        ...
        bool go_home_left(){
                ...
                if(largest_difference(left_home_values_,
                        ...
                        _pr2_mover->group->setStartState(*_pr2_mover->group->getCurrentState());

                        _pr2_mover->group->setJointValueTarget(_home_variable_values);

                        if(_pr2_mover->group->plan(_group_plan)){
                                _start_pub_joint_fb = false;
                                back_home = _pr2_mover->group->execute(_group_plan);
                                _pr2_mover->group->setStartState(*_pr2_mover->group->getCurrentState());
                            }
                    }
                else
        ...
        bool go_home_right(){
                ...
                if(largest_difference(right_home_values_,
                        ...
                        _pr2_mover->secondary_group->setStartState(*_pr2_mover->secondary_group->getCurrentState());

                        _pr2_mover->secondary_group->setJointValueTarget(_secondary_home);

                        if(_pr2_mover->secondary_group->plan(_group_plan)){
                                _start_pub_joint_fb = false;
                                back_home = _pr2_mover->secondary_group->execute(_group_plan);
                                _pr2_mover->secondary_group->setStartState(*_pr2_mover->secondary_group->getCurrentState());
                            }
                    }
                else
        ...
        std::vector<geometry_msgs::PoseStamped> get_random_targets_around_sphere(int number_target, std::vector<double> my_goal){
                ...
                for(int i = 0; i < number_target; i++){
                        ...
                        extended_pose.pose.position.x += _approach_radius + _extension_distance;
        ...
        bool reverse_back_trajectory(moveit::planning_interface::MoveGroup::Plan& a_plan, std::string arm){
                ...
                if(strcmp("left_arm", arm.c_str()) == 0){
                        moveit::core::robotStateToRobotStateMsg(*_pr2_mover->group->getCurrentState(), _revers_plan.start_state_);
                        return(_pr2_mover->group->execute(_revers_plan));
                    }
                else{
                        moveit::core::robotStateToRobotStateMsg(*_pr2_mover->secondary_group->getCurrentState(), _revers_plan.start_state_);
                        return(_pr2_mover->secondary_group->execute(_revers_plan));
                    }
            }

        //transform approach points and final goals to the planning frame "odom_combined"
        ...
        void execute(const pose_goalGoalConstPtr& poseGoal)
            {
                ...
                while(!complete_plan && iteration < _number_of_trials){
                        ...
                        //                        try{
                        //                            _listener.lookupTransform("odom_combined", "goal_frame", ros::Time(0), _transform);
                        //                        }
                        //                        catch (tf::TransformException &ex) {
                        //                                ROS_ERROR("%s",ex.what());
                        //                                ros::Duration(1.0).sleep();
                        ...
                        if(strcmp("left_arm", _arm.c_str()) == 0){
                                _pr2_mover->group->clearPoseTargets();
                                _pr2_mover->group->setStartState(*_pr2_mover->group->getCurrentState());
                                _pr2_mover->group->setPoseTargets(_transformed_target);
                                plan_result = _pr2_mover->group->plan(test_plan);
                                _start_state_second_trajectory = _pr2_mover->group->getCurrentState();
                            }
                        else{
                                _pr2_mover->secondary_group->clearPoseTargets();
                                _pr2_mover->secondary_group->setStartState(*_pr2_mover->secondary_group->getCurrentState());
                                _pr2_mover->secondary_group->setPoseTargets(_transformed_target);
                                plan_result = _pr2_mover->secondary_group->plan(test_plan);
                                _start_state_second_trajectory = _pr2_mover->secondary_group->getCurrentState();
                            }
                        ...
                        if(plan_result){
                                ...
                                if(strcmp("left_arm", _arm.c_str()) == 0)
                                    tf::poseEigenToMsg(_start_state_second_trajectory->getGlobalLinkTransform("l_gripper_tool_frame"), winning_pose);
                                else
                                    tf::poseEigenToMsg(_start_state_second_trajectory->getGlobalLinkTransform("r_gripper_tool_frame"), winning_pose);
                                ...
                                double fraction;
                                if(strcmp("left_arm", _arm.c_str()) == 0){
                                        _pr2_mover->group->setStartState(*_start_state_second_trajectory);
                                        fraction = _pr2_mover->group->computeCartesianPath(waypoints, 0.01, 0.0, robot_trajectory);
                                    }
                                else{
                                        _pr2_mover->secondary_group->setStartState(*_start_state_second_trajectory);
                                        fraction = _pr2_mover->secondary_group->computeCartesianPath(waypoints, 0.01, 0.0, robot_trajectory);
                                    }
                                ...
                                if(fraction >= _straight_line_threshold){
                                        complete_plan = true;
                                        if(strcmp("left_arm", _arm.c_str()) == 0)
                                            _pr2_mover->group->execute(test_plan);
                                        else
                                            _pr2_mover->secondary_group->execute(test_plan);
                                        ...
                                        if(strcmp("left_arm", _arm.c_str()) == 0){
                                                moveit::core::robotStateToRobotStateMsg(*_pr2_mover->group->getCurrentState(), test_touch_plan.start_state_);
                                                _pr2_mover->group->setStartState(*_pr2_mover->group->getCurrentState());
                                                _touch_object_success = _pr2_mover->group->execute(test_touch_plan);
                                            }
                                        else{
                                                moveit::core::robotStateToRobotStateMsg(*_pr2_mover->secondary_group->getCurrentState(), test_touch_plan.start_state_);
                                                _pr2_mover->secondary_group->setStartState(*_pr2_mover->secondary_group->getCurrentState());
                                                _touch_object_success = _pr2_mover->secondary_group->execute(test_touch_plan);
                                            }
    ...
    private:
        ...
        std::unique_ptr<ros::ServiceClient> _move_pr2_arm;
        ...
        std::string _parent_frame = "/odom_combined";
        ...
        pr2_mover_utils::move_pr2_arm::Request _motion_request;
        pr2_mover_utils::move_pr2_arm::Response _motion_response;
        pr2_mover::PR2_Mover::Ptr _pr2_mover;
